function varargout = vma_continuous_cuecascade(varargin)
...
im_list_B = cell(1, 4);
im_list_B{1} = imread('afasa9.jpg');
im_list_B{2} = imread('afasa10.jpg');
im_list_B{3} = imread('afasa11.jpg');
im_list_B{4} = imread('afasa12.jpg');

% CUE_TIME = .250; %sec
STIM_A_TIME_DUR = .25; %sec
STIM_B_TIME_DUR = .25;%STIM_A_TIME_DUR;
...
%             curr_target = home_position;
            ...
            STIM_ISI_DUR = .5;%(RET_TIME + TR_TIME) - WT_TIME - prescribed_PT(i_tr) - STIM_A_TIME_DUR - STIM_B_TIME_DUR;
                                    ...
                                    if sub_entrance == 1
                                        switch trial_type(i_tr)
                                            case 2
                                                ...
                                                temp_tx = ...
                                                    Screen('MakeTexture', win, im_list_B{trial_stimA_numbers(i_tr)});
                                                tx_size = size(im_list_B{trial_stimA_numbers(i_tr)});
                                                screen_picDim_buff{1, k_pic_buff} = [0; 0; tx_size(2); tx_size(1)];
                                                screen_picDim_buff{2, k_pic_buff} = [TEXT_LOC(1) - TEXT_SIZE; TEXT_LOC(2) - TEXT_SIZE;...
                                                 TEXT_LOC(1) + TEXT_SIZE; TEXT_LOC(2) + TEXT_SIZE];
                                                screen_pic_buff{k_pic_buff} = temp_tx;
