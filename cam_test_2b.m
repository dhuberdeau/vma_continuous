...
while(~keyIsDown)
    ...
    imperm = permute(imtext([3,2,1],:,:), [3 2 1]);
    imtext = undistortImage(imperm, camera_params);
    
    img = imtext(:, 1:DISC_SIZE:end, 1:DISC_SIZE:end);
%     img = permute(img_([3,2,1], :,:), [3,2,1]);
    b = rgb2hsv(img);

    im_r = inRange(b, [RMAX 1 1], [RMIN 0.5 0.5]);

    trk_y_rd = (nanmedian(ind1_d(im_r)));
    trk_x_rd = (nanmedian(ind2_d(im_r)));
...
%     trk_x_r = (res1 - trk_x_rd)*screen_dims(1)/res1; % for scanner setup
    ...
    tex2 = Screen('MakeTexture', win, imtext);
